...
public class DummyPlugin {
	...
	public void OnBlockBreak(ChangeBlockEvent.Break event, @Root Player p) {
		...
		Text msg = Text.builder("Player ").append(
				Text.builder(player.getName()).color(TextColors.GOLD).style(TextStyles.BOLD).build()).append(
				Text.builder(" has broken block at location ").build()).append(
				Text.builder(location.getX() + " " + location.getY() + " " + location.getZ()).style(TextStyles.BOLD).build()).build();
